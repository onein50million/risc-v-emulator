...
impl Processor{
    ...
    pub fn process(&mut self) -> ProcesserResult {
        ...
        let instruction = self.get_bytes(self.program_counter as usize, 4).try_into().unwrap();
        ...
        match opcode {
            ...
            opcodes::LOAD => {
                ...
                match i_format.funct3{
                    0b000 =>{
                        //lb
                        let value = i8::from_le_bytes(self.get_bytes(address as usize, 1).try_into().unwrap());
                        self.registers[i_format.rd as usize] = u64::from_le_bytes((value as i64).to_le_bytes());
                    }
                    0b001 => {
                        //lh
                        let value = i16::from_le_bytes(self.get_bytes(address as usize, 2).try_into().unwrap());
                        self.registers[i_format.rd as usize] = u64::from_le_bytes((value as i64).to_le_bytes());
                    }
                    0b010 => {
                        //lw
                        let value = i32::from_le_bytes(self.get_bytes(address as usize, 4).try_into().unwrap());
                        self.registers[i_format.rd as usize] = u64::from_le_bytes((value as i64).to_le_bytes());

                    }
                    0b011 =>{
                        //ld
                        let value = i64::from_le_bytes(self.get_bytes(address as usize, 8).try_into().unwrap());
                        self.registers[i_format.rd as usize] = u64::from_le_bytes(value.to_le_bytes());
                    }
                    0b100 => {
                        //lbu
                        let value = u8::from_le_bytes(self.get_bytes(address as usize, 1).try_into().unwrap());
                        self.registers[i_format.rd as usize] = value as u64;
                    }
                    0b101 => {
                        //lhu
                        let value = u16::from_le_bytes(self.get_bytes(address as usize, 2).try_into().unwrap());
                        self.registers[i_format.rd as usize] = value as u64;
                    }
                    0b110 => {
                        //lwu
                        let value = u32::from_le_bytes(self.get_bytes(address as usize, 4).try_into().unwrap());
                        self.registers[i_format.rd as usize] = value as u64;
                    }
                    _ => {self.unimplemented_instruction(instruction);}
                }
            ...
            opcodes::STORE =>{
                ...
                match s_format.funct3{
                    0b000 => {
                        self.set_bytes(address as usize, 1, self.registers[s_format.rs2 as usize].to_le_bytes().try_into().unwrap());
                    }
                    0b001 => {
                        self.set_bytes(address as usize, 2, self.registers[s_format.rs2 as usize].to_le_bytes().try_into().unwrap());

                    },
                    0b010 => {
                        self.set_bytes(address as usize, 4, self.registers[s_format.rs2 as usize].to_le_bytes().try_into().unwrap());

                    },
                    0b011 => {
                        //SD
                        self.set_bytes(address as usize, 8, self.registers[s_format.rs2 as usize].to_le_bytes().try_into().unwrap());
            ...
            opcodes::ENVIRONMENT => {
                match (instruction & !0b1111111) >> 20{
                    0b0 => {
                        //ECALL
                        let stack_pointer = self.registers[2] as usize;
                        let system_call_type = self.get_bytes(stack_pointer,8).try_into().unwrap();
                        let system_call_type = u64::from_le_bytes(system_call_type);

                        match system_call_type{
                            1 =>{
                                let string_address = self.get_bytes(stack_pointer+8, 8).try_into().unwrap();
                                let string_address = u64::from_le_bytes(string_address);

                                let mut string = vec![];

                                for i in (string_address as usize)..{
                                    let byte = self.get_bytes(i,1)[0];
                                    if byte == 0x0{
                                        break;
                                    }
                                    string.push(byte);
                                ...
                                let num_arguments = u64::from_le_bytes(self.get_bytes(stack_pointer + 16,8).try_into().unwrap()) as usize;
                                let argument_address = u64::from_le_bytes(self.get_bytes(stack_pointer + 24,8).try_into().unwrap()) as usize;
                                for i in 0..num_arguments{
                                    arguments.push(i64::from_le_bytes(self.get_bytes(argument_address + i*8,8).try_into().unwrap()));
                                }
        ...
        return ProcesserResult::Continue;
